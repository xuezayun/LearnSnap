...
RecPredictor::RecPredictor(const std::string &modelDir, const int cpuThreadNum,
  ...
  predictor_ =
      paddle::lite_api::CreatePaddlePredictor<paddle::lite_api::MobileConfig>(
          config);
  if (!predictor_) {
    throw std::runtime_error("failed to create recognition predictor");
  }
}

void RecPredictor::Preprocess(const cv::Mat &srcimg) {
  if (srcimg.empty() || srcimg.rows < 1 || srcimg.cols < 1) {
  ...
  if (resize_img.empty()) {
    throw std::runtime_error("recognition resize failed");
  }
