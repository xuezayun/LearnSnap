...
cv::Mat ClsResizeImg(cv::Mat img) {
  ...
  if (img.empty() || img.rows < 1 || img.cols < 1) return cv::Mat();
...
ClsPredictor::ClsPredictor(const std::string &modelDir, const int cpuThreadNum,
  ...
  predictor_ =
      paddle::lite_api::CreatePaddlePredictor<paddle::lite_api::MobileConfig>(
          config);
  if (!predictor_) {
    throw std::runtime_error("failed to create classification predictor");
  }
}

void ClsPredictor::Preprocess(const cv::Mat &img) {
  if (img.empty() || img.rows < 1 || img.cols < 1) {
    throw std::runtime_error("classification image is empty");
  }
  ...
  if (resize_img.empty()) {
    throw std::runtime_error("classification resize failed");
  }
