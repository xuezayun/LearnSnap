...
static int AlignSide(int value) {
...
cv::Mat DetResizeImg(const cv::Mat img, int max_size_len,
  ...
  if (img.empty() || img.cols < 1 || img.rows < 1) return cv::Mat();
  int w = img.cols;
  int h = img.rows;
  float ratio = 1.f;
  int max_wh = w >= h ? w : h;
  ...
  int resize_h = AlignSide(static_cast<int>(float(h) * ratio)); // NOLINT
  int resize_w = AlignSide(static_cast<int>(float(w) * ratio)); // NOLINT
  cv::Mat resize_img;
...
DetPredictor::DetPredictor(const std::string &modelDir, const int cpuThreadNum,
  ...
  predictor_ =
      paddle::lite_api::CreatePaddlePredictor<paddle::lite_api::MobileConfig>(
          config);
  if (!predictor_) {
    throw std::runtime_error("failed to create detection predictor");
  }
}
...
void DetPredictor::Preprocess(const cv::Mat &srcimg, const int max_side_len) {
  ...
  if (img.empty()) {
    throw std::runtime_error("detection resize failed");
  }
  cv::Mat img_fp;
